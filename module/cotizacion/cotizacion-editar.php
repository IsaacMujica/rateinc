...
if (!isset($_GET['folio'])) {
    ...
    if($detalle->cot_folio_padre3 == "" || $detalle->cot_folio_padre3 == null){
        $titulo = 'COTIZACIÓN N° ' . $folio;   
    }else{

        if($detalle->cot_folio_padre2 == "" || $detalle->cot_folio_padre2 == null){
            $titulo = 'COTIZACIÓN N° ' . $detalle->cot_folio_padre3;
        }else{
            $titulo = 'COTIZACIÓN N° ' . $detalle->cot_folio_padre2;
        }
    }
                                ...
                                <table class="table" id="tblOfertaEconomica">
                                    ...
                                    <?php foreach ($items as $key => $value) { ?>
                                        ...
                                        <tr>
                                            ...
                                            <td><?=$value->cotpro_codigo_sence?></td>
                                            <td><?=$value->cotpro_descripcion?></td>

                                            <td>
                                                <input id="txtDetExt_<?=$value->cotdet_producto_id?>" name="txtDetExt_<?=$value->cotdet_producto_id?>" type="text" class="form-control" value="<?=$value->cotdet_detextra?>" maxlength="50">
                                            </td>
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-4">
                                    ...
                                    <select class="form-control" name="selectSede" id="selectSede" disabled>
                                        <option value="">Seleccione sede</option>
                                        <?php
                                        foreach ($listSede as $key => $valor) {
                                            $selected = $detalle->cot_sede_id == $valor->id_sede ? "selected" : "";
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <textarea name="txtObsAdicional" id="txtObsAdicional" class="form-control" cols="20"
                                      rows="2"><?= $detalle->cot_obs_adicional ?></textarea>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    function guardarCotizacion() {
        ...
        if(idSedeCot == 0 || idSedeCot == "" || idSedeCot == undefined){
            idSedeCot = "<?= $detalle->cot_sede_id; ?>";            
        }
        ...
        //verificacion de folio padre con guion
        var folio_padre = "<?=$folio?>";
        var folio_padre2 = "<?=$detalle->cot_folio_padre2?>";
        var folio_padre3 = "<?=$detalle->cot_folio_padre3?>";


        if(folio_padre2 == null || folio_padre2 == "" || folio_padre2 == undefined){
            //folio_padre = "<?=$folio?>";

            if(folio_padre3 == null || folio_padre3 == "" || folio_padre3 == undefined){
                folio_padre = "<?=$folio?>";
            }else{
                folio_padre = "<?=$detalle->cot_folio_padre3?>";
            }
            //folio_padre = "<?=$detalle->cot_folio_padre3?>";
        //}//else{
            //folio_padre = "<?=$detalle->cot_folio_padre2?>";
            //folio_padre = "<?=$folio?>";
        //}

        //else if(folio_padre3 == null || folio_padre3 == "" || folio_padre3 == undefined){
            ...
            /*if(folio_padre2 == null || folio_padre2 == ""){
                folio_padre = "<?=$folio?>";
            }else{
                folio_padre = "1";
            }*/
            ...
            //falta otra validacion ?
            //folio_padre = "<?=$detalle->cot_folio_padre2?>";
            //folio_padre = "<?=$detalle->cot_folio_padre2?>";
            //folio_padre = folio_padre2;
            //folio_padre = "<?=$folio?>";
            //folio_padre = "1";
        }
        ...
        //var folio_padre = "<?=$folio?>";
        console.log(folio_padre);
        //console.log(folio_padre);
        //console.log(folio_padre);
        //console.log("asd");


        var folio_padre_final = "";
        var search_seg = folio_padre.indexOf("-");

        console.log("search: "+search_seg);

        if(search_seg < 0 || search_seg == "-1"){

           folio_padre_final = folio_padre+"-1";

        }else if(search_seg > 0){

            var folio_gen = folio_padre.split("-");
            var parte1 = folio_gen[0];
            var parte2 = parseInt(folio_gen[1])+1;
            folio_padre_final = parte1+"-"+parte2;
            
        }

        //let folio_final = String(folio_padre_final);

        console.log("final: "+folio_padre_final);
        ...
        $.ajax({
            ...
            data: "acc=4&" + cotizacion
                ...
                + "&cmbReferencia=" + cmbReferencia
                + "&idEmpresa=" + '<?= $detalle->cot_empresa_id ?>'
                + "&idUsuario=<?=$_SESSION['USUARIO_ID']?>"
                + "&idSede=" + idSedeCot
                + "&obsAdic=" + obsAdic
                //+ "&idCotizacion=<?=$folio?>"
                + "&idCotizacion=" + folio_padre_final
            ...
            success: function (data) {
                ...
                if (response.respuesta == "OK") {
                    alertOk("Correcto", response.mensaje, function () {
                        ...
                        setTimeout(function () {
                            //location.reload();
                            location.href = "../cotizacion/index.php";
                        }, 1000);
                    });
                } else {
